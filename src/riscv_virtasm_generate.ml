...
let convert_single name body terminator (inst: Riscv_ssa.t) =
  ...
  | Slli { rd; rs; imm } ->
      let r = ({ rd = slot_v rd; rs1 = slot_v rs; imm } : Slots.i_slot) in
      (match rs.ty with
      | T_uint | T_int | T_optimized_option { elem = T_char | T_bool | T_unit | T_byte } -> Vec.push body (Inst.Slliw r)
      | T_uint64 | T_int64 | T_optimized_option { elem = T_uint | T_int } -> Vec.push body (Inst.Slli r)
