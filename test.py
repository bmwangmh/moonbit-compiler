...
with DirContext("test"):
    ...
    for src in cases:
        ...
        os.system(f"moonc build-package src/{src}/{src}.mbt -is-main -std-path {bundled} -o build/{src}/{src}.core")

        # Linkage emits target code.
        ret = os.system(f"{debug} moonc link-core {bundled}/core.core build/{src}/{src}.core -o build/{src}/{src} -pkg-config-path {src}/{src}/moon.pkg.json -pkg-sources {core}:{src} -target {target}")
        ...
        if not args.compile_only:
            os.system(f"build/interpreter build/{src}/{src}.ssa > build/{src}/output.txt 2> build/{src}/debug.txt")
            diff = os.system(f"diff build/{src}/output.txt src/{src}/{src}.ans")
